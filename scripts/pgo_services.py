...
def handle_restart_mapping(req):
    ...
    # os.spawnl(os.P_NOWAIT, "rosrun aloam_velodyne alaserPGO")
...
def handle_save_pcd(req):
    ...
    data_dir = "../PCD/" # should end with / 
    scan_dir = data_dir + "Scans"
    scan_files = os.listdir(scan_dir) 
    scan_files.sort()

    scan_idx_range_to_stack = [0, len(scan_files)-5] # if you want a whole map, use [0, len(scan_files)]
    node_skip = 1
...
def pgo_services():
    ...
    rospy.Service('restart_mapping', Empty, handle_restart_mapping)
    rospy.Service('save_map', Empty, handle_save_pcd)
    rospy.spin()
